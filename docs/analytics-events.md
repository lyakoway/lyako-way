...
В заявках Telegram / Email уходят оба идентификатора устройства:

- **ClientID Метрика** → раздел «Посетители», фильтр по ClientID
- **ClientID GA4** → GA4: **Исследования → Шаблоны → Исследование пользователей** (User explorer), поиск / фильтр по Client ID

Формат в Telegram: отдельные строки `ClientID Метрика` и `ClientID GA4`; в письме — те же строки в конце текста сообщения.

UserID (хеш) в GA4 смотрите после того, как `user_id` успел уйти: отчёты по User-ID / исследования с измерением User ID (данные с задержкой).

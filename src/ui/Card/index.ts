...
export const Article = styled.article<{ $scene?: boolean }>`
  ...
  /* Навбар на десктопе абсолютно позиционирован в правом-верхнем углу секции
     и широкий (7 пунктов). Чтобы заголовок/крошки не налезали на него,
     опускаем контент секции ниже навбара. Для «Дома» (сцена) не нужно. */
  ${({ $scene }) =>
    !$scene &&
    css`

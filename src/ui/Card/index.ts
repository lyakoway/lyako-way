...
export const Article = styled.article<{ $scene?: boolean }>`
  ...
  /* Только в двухколоночной раскладке (≥1250px) навбар абсолютно позиционирован
     в правом-верхнем углу секции — опускаем контент ниже, чтобы заголовок/крошки
     не налезали. На <1250px навбар — верхняя панель, отступ не нужен.
     Для «Дома» (сцена) не нужно. */
  ${({ $scene }) =>
    !$scene &&
    css`
      @media (min-width: 1250px) {

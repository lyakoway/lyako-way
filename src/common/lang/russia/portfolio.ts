...
export const propsPortfolioList: PortfolioListProps[] = [
  {
    ...
    tagline:
      "Прод внутренний, в МТС. Публичное демо и метрики на этой странице — тестовый корпус.",
    metricsLine:
      "МТС, внутренний прод: ~2 000 документов · ~12 команд · ~200 вопросов/день · публичный сайт = тестовый корпус (held-out Recall@1 87%)",
    ...
    keyResultsNote:
      "Прод внутренний, в МТС (~2 000 документов, ~20 тыс. чанков). Публичное демо и латентность на этой странице — тестовый корпус (6 файлов → 12 чанков), не продовые документы.",
    keyResultsLimitation:
      "Продовый корпус внутренний и не публикуется.",
    technologies: [
      "Python",
      "FastAPI",
      "RAG",
    ...
    aiEngineering: {
      ...
      useCasesIntro:
        "RAG-чат закрывает типовую боль: знания заперты в десятках PDF, Word и Excel, и люди тратят часы на их ручной перебор. В production в МТС система обслуживает ~2 000 внутренних документов (~20 тыс. чанков) для ~12 команд (~200 вопросов/день). Живое демо и латентность на этой странице — публичный тестовый пак (6 файлов), не продовый корпус.",
      useCases: [
        {
      ...
      tables: [
        ...
        },
        {
          ...
          footnote:
            "Публичный тестовый корпус, не продовые данные МТС. Строка индексации: 6 файлов → 12 чанков. В production в МТС — ~2 000 внутренних документов (~20 тыс. чанков). TTFT — время до первого токена. GLM-4.5-flash (бесплатное поколение) на том же пайплайне даёт TTFT 25–50 с — поэтому в рекомендациях её нет.",
        },
        {
        ...
        items: [
          {
            ...
            text: "~2 000 внутренних документов (~20 тыс. чанков), ~12 команд, ~200 вопросов/день. Метрики качества на этой странице совмещают масштаб прода МТС и публичный тестовый пак для латентности.",
          },
          {
            title: "Streaming",
      ...
      findings: [
        "Языковые «двойники» — главная ловушка мультиязычных корпусов. Эмбеддинги выравнивают RU и EN, и русский вопрос может поднять английский документ (vector-only Recall@1 53%). Лексический сигнал BM25 в fusion — не опция, а необходимость (+34 п.п. к Recall@1).",
        "Held-out retrieval (~180 запросов) — качество: гибрид BM25 + RRF держит Recall@1 на 87%. Смешанные языковые запросы — follow-up (словарь синонимов RU/EN и мультиязычный реранкер), не headline-метрика.",
        ...
        "Автооценка качества ответов: LLM-as-judge оценил held-out ответы — 5.0 по всем осям (нет галлюцинаций, цитаты корректны). Для строгой оценки нужен судья другой семьи.",
  ...
  },
  {
    ...
    keyResultsNote:
      "Прод внутренний, в МТС. Публичное демо и SQL-оценка на этой странице — тестовый набор (RideGo ~21 тыс. поездок), не продовые базы.",
    keyResultsLimitation:
      "Продовые базы внутренние. Headline-качество SQL — held-out нормализованная точность результата (~85%), не strict exact-match.",
    technologies: [
      "Python",
      "FastAPI",
      "SQLAlchemy",
    ...
    aiEngineering: {
      ...
      tables: [
        ...
        },
        {
          ...
          footnote:
            "Held-out SQL-оценка на GLM-4.6. Headline-качество — нормализованная точность результата (~85%): алиасы, округления и ORDER BY канонизируются. Strict exact-match в заголовок не ставится.",
        },
        {

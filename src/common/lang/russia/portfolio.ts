...
export const propsPortfolioList: PortfolioListProps[] = [
  {
    ...
    portfolioText:
      "AI-ассистент с Retrieval-Augmented Generation: загружаете PDF, Word или Excel и задаёте вопросы по содержимому — ответы приходят со ссылками на страницы-источники.\nДемо-режим (offline) работает сразу, без ключей.;Для живых ответов подключены бесплатные модели GLM (GLM-4.5 Flash и GLM-4.6, Z.ai) — прямо в списке «Модель».;Также доступны OpenAI (GPT), Anthropic (Claude) и локальные модели через Ollama.\nBackend построен на FastAPI, ChromaDB (векторный поиск) и эмбеддингах.;Frontend — React 19 и TypeScript (Vite).",
    features: [

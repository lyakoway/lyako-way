...
export const propsPortfolioList: PortfolioListProps[] = [
  {
    ...
    aiEngineering: {
      ...
      useCasesIntro:
        "RAG-чат закрывает типовую боль: знания заперты в десятках PDF, Word и Excel, и люди тратят часы на их ручной перебор. Приложение превращает документы в разговор — вопрос на естественном языке, ответ с точной ссылкой на файл и страницу. Несколько сценариев, где это уже работает:",
      useCases: [
        {
        ...
        },
        {
          ...
          detail:
            "Новичок спрашивает ассистента с первого дня, не читая вики целиком; фильтры по категориям и документам в API ограничивают зону поиска нужным отделом — база для сценариев «подрядчик видит только свои документы».",
        },
      ],
      ...
      tables: [
        ...
        },
        {
          ...
          rows: [
            ...
            { cells: ["RAG-ответ, Llama 3.2 3B локально (Ollama, CPU): первый токен / полностью", "1,2–3,6 с / 2,5–5,4 с"] },
            { cells: ["Llama 3.2 3B: холодный старт (загрузка 2 ГБ модели в RAM)", "+13,7 с к первому токену"] },
        ...
        },
        {
          ...
          footnote:
            "Ответы и судья — glm-4.5-flash, поиск гибридный. Ответов с оценкой ≤3: 0 из 24. Судья той же GLM-семьи — самосуд снисходителен; для строгой оценки нужен судья другой семьи.",
        },
      ],
      ...
      findings: [
        ...
        "Автооценка качества ответов: LLM-as-judge оценил 24/24 ответов — 5.0 по всем осям (нет галлюцинаций, цитаты корректны). Но судья той же GLM-семьи, что и отвечающая модель: самосуд снисходителен — для строгой оценки нужен судья другой семьи.",
      ],
      gapsTitle: "Честные пробелы",
      gaps: [
        "A/B промптов ещё не проводилось; LLM-as-judge реализован (24/24 ответов оценены), но судья той же GLM-семьи, что и отвечающая модель, — для строгой оценки нужен судья другой семьи.",

  ...
  },
  {
    ...
    aiEngineering: {
      ...
      tables: [
        ...
        {
          title: "Замеры латентности — живой прогон по моделям",
          columns: ["Модель", "План (LLM)", "Выполнение (БД)", "Ответ (LLM)", "Итого", "SQL ok"],
          rows: [
            { cells: ["GLM-5.2 (Z.ai)", "4.9 с", "14 мс", "13.6 с", "~18.5 с", "✓"], highlight: true },
            { cells: ["GLM-4.6 (Z.ai)", "12.1 с", "7 мс", "10.3 с", "~22.4 с", "✓"] },
            { cells: ["GLM-5.3-flash (Z.ai)", "9.0 с", "—", "5.4 с", "~14.4 с", "✗"] },
            { cells: ["GLM-5.3 (Z.ai)", "7.9 с", "—", "4.2 с", "~12.2 с", "✗"] },
          ],
          footnote:
            "Единичный прогон одного вопроса через полный цикл (план → БД → ответ) — латентность внешнего API варьируется между запусками. Скрипт замера: python scripts/latency_benchmark.py.",
        },
        {

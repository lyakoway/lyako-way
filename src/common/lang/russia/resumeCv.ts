  ...
  experience: [
    {
      ...
      resultGroups: [
        ...
        },
        {
          ...
          items: [
            "Продукт готов и используется в компании.",
            "Сокращает время аналитика с 2 часов до 2 минут — сохранённый запрос, например «продажи по регионам за месяц», запускается как сценарий одной кнопкой.",
            "Agent loop (ReAct) с self-correction — агент сам находит ошибку SQL и переписывает запрос.",
            "Execution trace через SSE — пошаговая работа агентов в реальном времени.",
            "Детерминированная аналитика — тренды, топ-N и аномалии считает Python-слой, а не LLM.",
          ],
        },
      ],
      stack: {

...
export const useInView = <T extends HTMLElement = HTMLDivElement>(
  ...
  // Стартуем ВИДИМЫМ: так контент отрисован уже в SSR-HTML и не «пропадает»,
  // пока на медленной сети грузится JS. Элементы ниже сгиба прячем ниже, в
  // layout-эффекте (до отрисовки, без мигания), чтобы проиграть появление
  // при скролле. Без JS всё просто остаётся видимым — прогрессивное улучшение.
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    // Элемент ниже сгиба: прячем его здесь, в layout-эффекте (синхронно, до
    // отрисовки — без мигания на быстрой сети), чтобы затем проиграть появление
    // при скролле. На медленной сети до этого момента он уже был показан из SSR,
    // но пользователь его не видел (он за сгибом) — промежутка не возникает.
    setState({ inView: false, animate: false });

    // Анимируем появление ТОЛЬКО при реальном вводе пользователя
    // (wheel/touch/клавиши). Событие `scroll` не подходит: его бросает и
    // программный скролл-наверх Next.js при переходе между страницами, и
    // досчёт вёрстки — тогда карточки «ехали» без скролла.
